...
export default (async function closeDoc(request, done) {
  ...
  try {
    ...
    const current_payment_date =
      doc.has('payment_date') && doc.get('payment_date').getTime
    ...
    const current_payment_amount = doc.has('payment_amount')
      ...
      : null;
    if (
      current_payment_amount !== paymentAmount ||
      current_payment_date !== paymentDateMS
    ) {
      paymentInfo = {
    ...
    const current_validation_date =
      doc.has('validation_date') && doc.get('validation_date').getTime
    ...
    const current_validation_amount = doc.has('validation_amount')
      ...
      : null;
    if (
      current_validation_date !== dateClosureMS ||
      current_validation_amount !== mtRapports
    ) {
      validationInfo = {
