...
const babelOptions = require('scripts/getBabelOptions')({
  ...
  plugins: ['transform-async-to-generator', 'transform-export-extensions'],
  modules: true,
  regenerator: false,
});

babelRegister(
  objectAssign(babelOptions, {
    comments: false,
    compact: true,
    babelrc: false,
    env: {
    ...
    value:
      key === 'SERVER_URL'
        ? config.parse_server_url ||
          `http://localhost:${config.server_port}${config.parse_server_mount_point}`
        : JSON.parse(vars[key]),
  });
});
