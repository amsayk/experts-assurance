...
function StateUser({ doc }) {
  ...
  return (
    <span style={{ marginLeft: 5, display: 'flex', alignItems: 'center' }}>
      ...
      <Link
        to={
          PATH_SETTINGS_BASE + '/' + PATH_SETTINGS_BUSINESS_USER + '/' + user.id
        }
      >
        <span
            ...
            updateQueries: {
              ...
              moreOpenDocs(prev, { mutationResult, queryVariables }) {
                ...
                if (prev && newDoc && oldDoc.state === 'OPEN') {
                  ...
                  if (index !== -1) {
                    const docs = prev.openDashboard.docs.filter(
                      doc => doc.id !== newDoc.id,
                    );
              ...
              getTimeline(prev, { mutationResult, queryVariables }) {
                ...
                if (prev && newActivities && newActivities.length) {
                  if (
                    ...
                    queryVariables.query.doc !== newDoc.id
                  ) {
                    return prev;
                  }
...
export default compose(injectIntl, withApollo, Connect)(StateLine);
