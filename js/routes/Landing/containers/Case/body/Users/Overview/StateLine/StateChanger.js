...
function getState(state, stateText, icon) {
  return (
    <div className={style.state} style={{ marginRight: 5 }}>
      <span
        className={cx(
          style[state === 'VALID' ? 'OPEN' : state],
          style.docStateToggle
        )}
      >
        {icon}
      </span>
...
class StateToggle extends React.Component {
  render() {
    ...
    if (
      ...
      doc &&
      doc.validation &&
      (doc.validation.date && doc.validation.amount !== null)
    ) {
      _state = 'VALID';
    }
    ...
    return (
      <Button
        className={cx(style.selectedStateButton, style.togglePickUser)}
...
class StateChanger extends React.Component {
  onToggle() {
    this.setState(
      ({ open, state }, { deletion }) => ({
        open:
          deletion || state === 'CLOSED' || state === 'CANCELED'
            ? false
            : !open,
      }),
      () => {}
  ...
  componentWillReceiveProps(nextProps) {
    if (this.state.state !== nextProps.state) {
      this.setState({
        ...
        keys: STATES.filter(
          key => STATE_LEVEL[key] > STATE_LEVEL[nextProps.state]
        ),
      });
    }
  }
  ...
  render() {
    ...
    return (
      <div className={style.filterGroup}>
        <div className={cx(style.pickUserDropdown)}>
...
export default compose(Connect)(StateChanger);
