...
class NatureSetter extends React.Component {
  state = {
    open: false,
  };
  constructor() {
  ...
  onSet(info) {
    ...
    if (!self.state.busy) {
      ...
      self.props.onSetNature({
        value: info.get('nature'),
      });
    }
  }
  onDelete() {
    ...
    const edit = canMutate
      ? <div className={style.docSetDTValidation} style={styles.action1}>
          <Dropdown open={this.state.open} onToggle={this.onToggle}>
            ...
            >
              {hasValue ? <PencilIcon size={18} /> : 'Initializer la nature…'}
            </Dropdown.Toggle>
            <Dropdown.Menu className={style.docSetDTValidationFormMenu}>
              <MenuItem
                ...
                onSubmit={this.onSet}
                initialValues={{
                  nature: hasValue ? doc.nature : null,
                }}
...
export default compose(injectIntl)(NatureSetter);
