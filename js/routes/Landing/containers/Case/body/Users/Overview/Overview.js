...
import { UndoIcon, TrashIcon } from 'components/icons/MaterialIcons';
                ...
                updateQueries: {
                  ...
                  openDocs(prev, { mutationResult, queryVariables }) {
                    ...
                    if (prev && newDoc && newDoc.state === 'OPEN') {
                      ...
                      const docs =
                        index !== -1
                          ? prev.openDashboard.docs.filter(
        ...
        onOk: () => {
          ...
          self.setState(
            ...
            async () => {
              ...
              const {
                data: { [isDeletion ? 'delDoc' : 'restoreDoc']: { error } },
              } = await self.props.client.mutate({
                refetchQueries: [
                  'getDoc',
                  'invalidDocs',
  ...
  render() {
    ...
    return (
      <div
        className={cx(
          style.overview,
          loading ? undefined : doc.deletion && style.deletedDoc,
        )}
      >
        <div className={cx(style.overviewContent, style.card)}>
              ...
              {loading
                ...
                : [
                    <div className={style.docRefNo}>
                      Dossier <b style={{ color: 'black' }}>{doc.refNo}</b>
                    </div>,
                    <div
                      onClick={this.copyKeyToClipboard}
                      className={style.docKeyBadge}
                    >
                      {doc.key}
                    </div>,
                  ]}
            ...
            <div className={style.deleteOrRestoreDocAction}>
              {(() => {
                ...
                if (doc.deletion) {
                  return (
                    <Button
                      onClick={this.onDeleteOrRestoreDoc}
                      className={style.deleteOrRestoreDocButton}
                      role='button'
                    >
                      <UndoIcon size={32} />
                    </Button>
          ...
          <div className={style.docContent}>
            ...
            {/*   doc={doc} */}
            {/* /> */}
            <StateLine loading={loading} doc={doc} user={user} />
            {/* <VehicleLine */}
            {/*   loading={loading} */}
            {/*   doc={doc} */}
            {/* /> */}
            {user.isAdmin ? <ManagerLine loading={loading} doc={doc} /> : null}
            <ClientLine label='Assureur' loading={loading} doc={doc} />
            <AgentLine loading={loading} doc={doc} />
            {/* <DTClosureLine */}
            {/*   loading={loading} */}
            {/*   doc={doc} */}
            {/* /> */}
            <DTSinisterLine loading={loading} doc={doc} />
            <DTMissionLine loading={loading} doc={doc} />
            <DTValidationLine loading={loading} doc={doc} currentUser={user} />
            <MTRapportsLine loading={loading} doc={doc} currentUser={user} />
            {(() => {
              if (loading) {
                return null;
              }
              ...
              return user.isAdmin || user.isManager(doc)
                ? <Payment loading={false} doc={doc} currentUser={user} />
                : null;
            })()}
            <NatureLine loading={loading} doc={doc} currentUser={user} />
            <PoliceLine loading={loading} doc={doc} currentUser={user} />
            {(() => {
              ...
              if (user.isAdmin || user.isManager(doc)) {
                return <DeletionLine loading={loading} doc={doc} />;
              }

              return null;
            })()}
            <LastActivityLine loading={loading} doc={doc} />
          </div>
        </div>
      </div>
    );
...
export default compose(injectIntl, withApollo, Connect, DataLoader.doc)(
  Overview,
);
