...
const CONFIRM_GRANT = <div style={style.confirmToastr}>
  <h5>Êtes-vous sûr?</h5>
  <div>
    Cet utilisateur aura accès aux dossiers.
  </div>
</div>;

const CONFIRM_REVOKE = <div style={style.confirmToastr}>
  <h5>Êtes-vous sûr?</h5>
  <div>
    Cet utilisateur ne pourra plus accèder aux dossiers.
  </div>
</div>;
    ...
    function doAction() {
      ...
      if (user) {
        ...
        self.setState({
          ...
          const { data: { revokeManagerAuthorization: { error } } } = await self.props.client.mutate({
    ...
    function doAction() {
      ...
      if (user) {
        ...
        self.setState({
          ...
          const { data: { authorizeManager: { error } } } = await self.props.client.mutate({
            refetchQueries     : ['settings_user__getUser'],
        ...
        <div style={styles.info}>
          {user.authorization
            ? [
              ...
              <span title={intl.formatDate(user.authorization.date)}>
                {' '}le {intl.formatDate(user.authorization.date)}{' à '}{intl.formatTime(user.authorization.date)}
              </span>,
            ]
          ...
          {currentUser.isAdmin ? [
            ...
            <div style={styles.action}>
              {user.authorization ? <Button className={style.authorizationBtn} bsStyle='link' onClick={this.onRevoke} role='button'>
                Révoquer son autorisation
                </Button> : <Button className={style.authorizationBtn} bsStyle='link' onClick={this.onGrantAuthorization} role='button'>
                Autoriser
              </Button>}
            </div>,
          ] : null}
