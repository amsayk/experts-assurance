...
export const UserIsAuthenticated = UserAuthWrapper({
  ...
  failureRedirectPath: (_, { location }) => {
    const redirect =
      location && location.query && location.query.redirect
...
UserIsAuthenticated.onEnter = store => {
  const connect = fn => (nextState, replaceState) =>
    fn(store, nextState, replaceState);
  return connect(UserIsAuthenticated._onEnter);
};

export const UserDidNotChangePassword = UserAuthWrapper({
  wrapperDisplayName: 'UserDidNotChangePassword',
  authSelector: state => state.get('user'),
  predicate: user => !user.getIn(['changePasswordAtNextLogin']),
  failureRedirectPath: PATH_SETTINGS_BASE + '/' + PATH_SETTINGS_CHANGE_PASSWORD,
});

UserDidNotChangePassword._onEnter = UserDidNotChangePassword.onEnter;
UserDidNotChangePassword.onEnter = store => {
  const connect = fn => (nextState, replaceState) =>
    fn(store, nextState, replaceState);
  return connect(UserDidNotChangePassword._onEnter);
};
...
export const UserIsAuthorized = UserAuthWrapper({
  ...
  predicate: () => {
    const user = getCurrentUser();
    if (user && !userHasRoleAll(user, Role_ADMINISTRATORS)) {
...
export const UserIsNotAuthorized = UserAuthWrapper({
  ...
  predicate: () => {
    ...
    if (user && !userHasRoleAll(user, Role_ADMINISTRATORS)) {
      return !!(
        user.get('authorization_date') && user.get('authorization_user')
      );
    }
    return true;
  },
...
export const onEnter = store =>
  connect(store)(
    ...
    EmailIsNotVerified._onEnter,
    UserIsNotAuthorized._onEnter,
    UserDidNotChangePassword._onEnter,
  );
...
export const connect = store => (...components) => {
  ...
  return connect(onEnterChain(...components));
};
