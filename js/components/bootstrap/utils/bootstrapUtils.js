...
import getLocalCSSClassName from './getLocalCSSClassName';
...
export function getClassSet(style, props) {
  const classes = {
    [getLocalCSSClassName(style, prefix(props))]: true,
  };

  if (props.bsSize) {
    const bsSize = SIZE_MAP[props.bsSize] || props.bsSize;
    classes[getLocalCSSClassName(style, prefix(props, bsSize))] = true;
  }

  if (props.bsStyle) {
    classes[getLocalCSSClassName(style, prefix(props, props.bsStyle))] = true;
  }
